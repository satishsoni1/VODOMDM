...
@if(session('bulk_device_summary'))
@php $bds = session('bulk_device_summary'); @endphp
<div class="alert alert-{{ count($bds['errors']) ? 'warning' : 'success' }} alert-dismissible fade show">
    <strong>Bulk Device Assignment:</strong>
    {{ $bds['assigned'] }} devices assigned, {{ $bds['skipped'] }} skipped.
    @if(count($bds['errors']))
    <ul class="mb-0 mt-2">@foreach($bds['errors'] as $err)<li class="small">{{ $err }}</li>@endforeach</ul>
    @endif
